...
import { outcomeCards } from "@/lib/discovery/outcomes";
...
export const Route = createFileRoute("/discover")({
  head: () => {
    const outcomes = outcomeCards();
    return {
      ...publicRouteMeta({ title, description, path: "/discover" }),
      scripts: [
        jsonLdScript(webPageJsonLd({ name: title, description, path: "/discover" })),
        jsonLdScript(itemListJsonLd(outcomes.map((outcome) => ({ name: outcome.title, path: outcome.to })))),
    ...
    <MarketingLayout>
      <PageSection>
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-2">
          {outcomes.map((outcome) => (
            <Link key={outcome.id} to={outcome.to} onClick={() => track("discovery_resolved", { props: { destination: outcome.to, outcome: outcome.id } })} className="group rounded-2xl border border-border bg-card p-5 transition-colors hover:border-primary/50 hover:bg-primary-soft">
              <div className="flex items-start justify-between gap-4"><h2 className="type-h3">{outcome.title}</h2><ArrowRight className="size-5 text-primary transition-transform group-hover:translate-x-0.5" aria-hidden="true" /></div>
              {outcome.label ? <p className="mt-2 type-overline text-primary">{outcome.label}</p> : null}
              <p className="mt-2 type-body-sm text-muted-foreground">{outcome.summary}</p>
            </Link>
          ))}
        </div>
      </PageSection>
      <PageSection className="pt-0">
        <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8">
          ...
          <div className="mt-6 flex flex-wrap gap-3"><Button asChild><Link to="/search"><Search aria-hidden="true" />Find storage</Link></Button><Button asChild variant="secondary"><Link to="/tools">Explore tools</Link></Button><Button asChild variant="secondary"><Link to="/how-it-works">How it works</Link></Button></div>
        </div>
      </PageSection>
    </MarketingLayout>
  );
