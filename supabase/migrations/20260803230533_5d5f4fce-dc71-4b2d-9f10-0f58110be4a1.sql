...
BEGIN
  ...
  IF FOUND THEN
    -- Re-emit trusted payment coordinates for EVERY claimed refund so a retry
    -- reconciles each one with Stripe (same idempotency key replays it).
    FOR v_refund IN
  ...
  IF v_started THEN
    INSERT INTO public.booking_cancellations (
      ...
      p_booking_id, v_first_payment, v_uid, v_role, p_reason, true, v_policy,
  ...
  INSERT INTO public.booking_cancellations (
    ...
    p_booking_id, v_first_payment, v_uid, v_role, p_reason, false, v_policy,
  ...
  FOR v_payment IN
    SELECT * FROM public.payments
     ...
     FOR UPDATE
  LOOP
    -- Only what remains unrefunded on THIS payment: an already fully refunded
    -- payment contributes nothing and is never refunded twice.
    ...
    IF (v_storage + v_fee) > 0 THEN
      -- A pending refund already in flight for this payment is respected by the
      -- partial unique index; skip it rather than fail the whole cancellation.
