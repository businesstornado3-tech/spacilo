...
export function parseRegistryId(id: string): { kind: RegistryKind; year: number; sequence: number } | null {
...
export function nextSequence(kind: RegistryKind, year: number, existing: readonly string[]): number {
  let highest = 0;
  for (const id of existing) {
    const parsed = parseRegistryId(id);
    if (parsed && parsed.kind === kind && parsed.year === year) highest = Math.max(highest, parsed.sequence);
  }
  return highest + 1;
}
