 ...
 *
 * Labels live outside the artwork so nothing can ever overlap the scene, the
 * walkway marker or each other.
 */
  ...
  return (
    <div className={cn("min-w-0", className)}>
      <div className="grid grid-cols-2 gap-2">
        <span className="justify-self-start rounded-full bg-surface px-2.5 py-1 type-badge text-muted-foreground">
          Before
        </span>
        <span className="justify-self-end rounded-full bg-success-soft px-2.5 py-1 type-badge text-success-soft-foreground">
          After
        </span>
      </div>
      ...
      <label className="mt-3 block">
        <span className="type-label">Drag to compare</span>
        <input
          type="range"
          min={0}
          max={100}
          ...
          className="mt-1.5 h-11 w-full accent-[var(--color-signal)]"
      ...
      <dl className="grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-surface p-3">
          ...
          <dd className="mt-0.5 type-card-title">{plan.before.floorAreaUsed.toFixed(1)}m²</dd>
        </div>
        <div className="rounded-xl bg-success-soft p-3">
          <dt className="type-overline text-success-soft-foreground">After — floor used</dt>
          <dd className="mt-0.5 type-card-title">{plan.after.floorAreaUsed.toFixed(1)}m²</dd>
        </div>
      </dl>
    </div>
  );
