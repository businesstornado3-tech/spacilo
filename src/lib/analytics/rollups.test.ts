...
const rows: RollupRow[] = [
  {
    ...
    event_name: ROLLUP_ALL_EVENTS,
    total_events: 10,
    public_events: 8,
    unique_visitors: 4,
    sessions: 6,
    public_unique_visitors: 3,
    public_sessions: 5,
  },
  {
    rollup_date: "2026-01-01",
    ...
    unique_visitors: 3,
    sessions: 4,
    public_unique_visitors: 2,
    public_sessions: 3,
  },
  {
    rollup_date: "2026-01-02",
    event_name: "page_view",
    total_events: 2,
    public_events: 2,
    unique_visitors: 2,
...
describe("analytics rollup helpers", () => {
  it("splits long rebuilds into bounded, contiguous chunks", () => {
    ...
    expect(
      Math.max(...chunks.map((chunk) => chunk.to.getTime() - chunk.from.getTime())),
    ).toBeLessThanOrEqual(MAX_REBUILD_DAYS * 86_400_000);
    expect(
      chunks.every(
        (chunk, index) => index === 0 || chunk.from.getTime() === chunks[index - 1]!.to.getTime(),
      ),
    ).toBe(true);
  });
