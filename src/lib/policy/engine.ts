...
function physicalDimension(spaceFit: {
  ...
  if (!spaceFit || typeof spaceFit.score !== "number") {
    return {
      ...
      detail: "Add your belongings and the space's measurements for a SpaceFit estimate.",
      reasons: [],
    };
  }
  if (spaceFit.compatible === false || spaceFit.score < 50) {
    return {
      ...
      detail: `SpaceFit estimates ${spaceFit.score}% — your belongings probably need more room than this space offers.`,
  ...
  if (spaceFit.score < 75) {
    return {
      ...
      detail: `SpaceFit estimates ${spaceFit.score}%. It's an estimate, not a measurement.`,
  ...
  return {
    ...
    detail: `SpaceFit estimates ${spaceFit.score}% based on what you've told us.`,
    reasons: [],
  };
}
