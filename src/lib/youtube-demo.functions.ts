 ...
 * honest "authorisation required" state.
 */
async function youtubeAccessToken(): Promise<{ token: string | null; detail: string }> {
  ...
  const { data: row } = await (supabaseAdmin as any)
    ...
    .eq("platform", "youtube")
    .maybeSingle();
  if (!row) return { token: null, detail: "No YouTube authorisation is stored yet. Connect the channel first." };

  const expired = row.expires_at ? Date.parse(row.expires_at) <= Date.now() + 30_000 : false;
  if (!expired) {
    try {
      return { token: await decryptToken(row.access_token_cipher), detail: "Stored authorisation in use." };
    } catch {
      return { token: null, detail: "The stored authorisation could not be read. Reconnect the channel." };
    }
  }

  if (!row.refresh_token_cipher) {
    return { token: null, detail: "The authorisation has expired and Google returned no refresh token. Reconnect the channel." };
  }
  const clientId = process.env["YOUTUBE_CLIENT_ID"];
  const clientSecret = process.env["YOUTUBE_CLIENT_SECRET"];
  if (!clientId || !clientSecret) {
    return { token: null, detail: "The Google application credentials are not configured on this server." };
  }
  try {
    ...
    if (!response.ok || !token) {
      return { token: null, detail: "Google refused to refresh the authorisation. Reconnect the channel." };
    }
    const expiresIn = typeof payload["expires_in"] === "number" ? payload["expires_in"] : 3600;
    await (supabaseAdmin as any).from("marketing_platform_tokens").update({
    ...
    return { token: null, detail: "The stored authorisation could not be refreshed. Reconnect the channel." };
  }
}
