...
export const Route = createFileRoute("/_authenticated/admin/marketing")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Internal EarnRoom marketing and growth intelligence console." },
        ...
        <div className="space-y-8">
          ...
          >
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => studio.settings.mutate({ pauseAllPublishing: !snapshot.settings.pauseAllPublishing })}
                className="min-h-11 rounded-lg border border-border px-3 type-nav text-muted-foreground hover:bg-secondary"
              >
                {snapshot.settings.pauseAllPublishing ? "Resume publishing" : "Pause all publishing"}
              </button>
              ...
              <div className="space-y-4">
                <div className="rounded-xl border border-border p-4">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Pill tone="neutral">{snapshot.today.status.replace(/_/g, " ").toLowerCase()}</Pill>
                    <Pill tone="neutral">Priority {Math.round(snapshot.today.score.priority)}/100</Pill>
                    <Pill
                      tone={snapshot.today.opportunity.evidenceClass === "REAL_MARKET_DEMAND" ? "good" : "warn"}
                    >
                  ...
                  <p className="mt-1 type-body-sm text-muted-foreground">{snapshot.today.opportunity.problem}</p>
                ...
                <div className="rounded-xl border border-border p-4">
                  <h4 className="type-h5">Story — {snapshot.today.story.format.replace(/_/g, " ").toLowerCase()}</h4>
                  ...
                  <ol className="mt-3 space-y-2">
                    {snapshot.today.story.scenes.map((scene) => (
                      <li key={scene.index} className="type-body-xs text-muted-foreground">
                        <span className="font-semibold text-foreground">{scene.seconds}s</span> — {scene.visual}
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {snapshot.today.assets.map((asset) => (
                    <div key={asset.id} className="rounded-xl border border-border p-4">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="type-body-sm font-semibold">{definition(asset.platform).label}</span>
                        <Pill tone={asset.state === "PUBLISHED" ? "good" : asset.state.includes("FAIL") ? "bad" : "warn"}>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => studio.decide.mutate({ campaignId: snapshot.today!.id, decision: "REJECT" })}
                    ...
                    <ul className="space-y-1">
                      {studio.publish.data.results.map((result) => (
                        <li key={result.platform}>
                          {definition(result.platform as never).label}: {result.state.replace(/_/g, " ").toLowerCase()} — {result.detail}
                        </li>
                      ))}
                    </ul>
                  </Alert>
          ...
          >
            {snapshot.demandPlaces.length === 0 ? (
              <p className="type-body-sm text-muted-foreground">
                No location intent in the period — campaigns are coming from search and awareness opportunities instead.
              </p>
            ) : (
              <ul className="space-y-1">
                {snapshot.demandPlaces.map((place) => (
                  <li key={place.slug} className="type-body-sm">
                    ...
                    <span className="text-muted-foreground">
                      {place.demandEvents} intent signal(s), {place.publishedSpaces} published space(s) · {place.priority.toLowerCase()} priority
                    </span>
                  </li>
                ))}
              </ul>
          ...
          <AdminSectionBlock id="history" title="Recent campaigns" note="Coverage, so the same place and topic are not repeated.">
              ...
              <ul className="space-y-1">
                {snapshot.recent.map((row) => (
                  <li key={row.id} className="type-body-sm">
                    ...
                    <span className="type-body-xs text-muted-foreground">{row.source.toLowerCase()}</span>
                  </li>
                ))}
              </ul>
            )}
              ...
              <ul className="space-y-1">
                {snapshot.insights.map((insight) => (
                  <li key={`${insight.dimension}:${insight.value}`} className="type-body-sm">
                    ...
                    <span className="text-muted-foreground">
                      ({insight.dimension}) index {insight.index} from {insight.samples} observation(s)
                    </span>
                  </li>
                ))}
              </ul>
